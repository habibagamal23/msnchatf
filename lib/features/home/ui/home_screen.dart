...
class HomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
        create: (context) => RoomsCubit(FireBaseData())..fetchuserandroom(),
        child: Scaffold(
          appBar: AppBar(
            leading: IconButton(
                onPressed: () {},
            ...
            if (state is RoomsLoaded) {
              return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    final otherid = state.rooms[index].members.firstWhere(
                        (id) => id != FireBaseData().myUid,
                        orElse: () => "unkown");

                    final userprofile =
                        context.read<RoomsCubit>().getuserprofile(otherid);
                    if (otherid == "unkown" || userprofile == null) {
                      return Container(child: Text("Unkonw"));
                    }

                    return UserCard(
                        room: state.rooms[index], userProfile: userprofile);
                  });
            }
